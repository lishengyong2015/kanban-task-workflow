...
python3 ~/.openclaw/workspace/skills/email-sender/scripts/send_email.py \
  ...
  --attach ~/.hermes/kanban/workspaces/${task_id}/research-findings.md \
  --body-type plain
```
...
message: "⊘ [blocked] ${task_number} Research完成，审核邮件已发送至审核人邮箱，请查阅"
```

**STEP 4** — 记录阶段状态后阻塞等待审核
```
# 更新 TASK.md 记录当前阶段
sed -i "s/current_phase:.*/current_phase: phase1/" ~/.hermes/kanban/workspaces/${task_id}/TASK.md

# 阻塞
kanban_block(reason="研究完成，邮件已发送至审核人邮箱，等待审核确认后 unblock")
```
...
terminal: python3 ~/.openclaw/workspace/skills/email-sender/scripts/send_email.py \
  ...
  --attach ~/.hermes/kanban/workspaces/${task_id}/implementation-plan.md \
  --body-type plain
```

**STEP 3** — 发送通知给创建者
```
send_message: target=notify_target, message="⊘ [blocked] ${task_number} 方案完成，审核邮件已发送至审核人邮箱，请查阅"
```

**STEP 4** — 记录阶段状态后阻塞等待审核
```
sed -i "s/current_phase:.*/current_phase: phase2/" ~/.hermes/kanban/workspaces/${task_id}/TASK.md
kanban_block(reason="方案完成，邮件已发送至审核人邮箱，等待审核确认后 unblock")
```
...
4. 记录阶段并阻塞：
```
sed -i "s/current_phase:.*/current_phase: phase3_5/" ~/.hermes/kanban/workspaces/${task_id}/TASK.md
...
```
sed -i "s/current_phase:.*/current_phase: phase4/" ~/.hermes/kanban/workspaces/${task_id}/TASK.md
kanban_block(reason="编译完成，邮件已发送至审核人邮箱，等待审核确认后 unblock")
```
